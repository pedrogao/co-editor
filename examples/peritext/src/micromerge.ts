...
export default class Micromerge {
    ...
    for (const inputOp of ops) {
      ...
      if (Array.isArray(obj) && Array.isArray(meta)) {
        if (inputOp.action === "insert") {
          let elemId =
            inputOp.index === 0 ? HEAD : getListElementId(meta, inputOp.index - 1, { lookAfterTombstones: true });
   ...
   */
  private makeNewOp(change: Change, op: DistributiveOmit<Operation, "opId">): { opId: OperationId; patches: Patch[] } {
    this.maxOp += 1;
  ...
  private applyListInsert(op: InsertOperation): Patch[] {
    ...
    let { index, visible } = op.elemId === HEAD ? { index: -1, visible: 0 } : this.findListElement(op.obj, op.elemId);
