    ...
    case "insert": {
      ...
      return {
        transaction: transaction.replace(
          index,
          index,
          new Slice(Fragment.from(schema.text(patch.values[0], getProsemirrorMarksForMarkMap(patch.marks))), 0, 0),
        ),
        startPos: index,
        endPos: index + 1,
      };
      ...
      if (txn.selectionSet) {
        state = state.apply(
          state.tr.setSelection(
            new TextSelection(state.doc.resolve(txn.selection.anchor), state.doc.resolve(txn.selection.head)),
          ),
        );
      }
      ...
      if (step.slice) {
        ...
        if (step.from !== step.to) {
          operations.push({
            ...
            count:
              contentPosFromProsemirrorPos(step.to, txn.before) - contentPosFromProsemirrorPos(step.from, txn.before),
          });
        }
